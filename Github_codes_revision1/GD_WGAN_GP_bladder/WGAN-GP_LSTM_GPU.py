...
output_dir = data_file + 'result/WGAN-GP_LSTM/epoch' + str(num_epochs)
...
for epoch in range(num_epochs):
    ...
    '''
    Dupdate_time_each, Gupdate_time_each, Ggen_time_each, Dpred_time_each, CNNPred_time_each = [], [], [], [], []
    ...
    summary_string = 'Epoch{0}/{1}: d_real_loss-{2:.2f},d_fake_loss-{3:.2f},d_total_loss-{4:.2f},G_score-{5:.2f},G_total_loss-{6:.2f},W_dist-{7:.2f},G_max_score-{8:.2f},unq_rate-{9:.2f}' \
        .format(epoch + 1, num_epochs, np.mean(d_real_losses), np.mean(d_fake_losses), np.mean(D_losses),
                np.mean(G_score), np.mean(G_losses), np.mean(W_dist), np.mean(G_max_score), np.mean(G_unq))
...
print('Training Time: '+ str(stop_train - start_train) + '(s)'
      ...
      + ', Total OtherTime: '+ str(stop_train - start_train - np.sum(array11) - np.sum(array12) - np.sum(array13) - np.sum(array14) - np.sum(array15)) + '(s)')
# save the model
