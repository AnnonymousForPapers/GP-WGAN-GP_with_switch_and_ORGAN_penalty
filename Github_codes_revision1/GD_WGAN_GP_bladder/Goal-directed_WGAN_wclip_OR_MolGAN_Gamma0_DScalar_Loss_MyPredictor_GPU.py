...
class Discriminator(nn.Module):
    ...
    def forward(self,input):  # input [N,seq_len,n_chars]
        ...
        # output = torch.sigmoid(output)   # apply sigmoid to constrain output in (0, 1) <- Add here
        return output
...
print(f"Total G params: {count_params(G):,}")
...
output_dir = data_file + 'result/GD_WGAN_wclip_OR_MolGAN_Gamma0_DScalar_Loss_MyPredictor/epoch' + str(num_epochs)
...
for epoch in range(num_epochs):
    ...
    '''
    Dupdate_time_each, Gupdate_time_each, Ggen_time_each, Dpred_time_each, CNNPred_time_each, Supdate_time_each, Spred_time_each = [], [], [], [], [], [], []
    d_fake_losses, d_real_losses, grad_penalties = [], [], []
    S_losses = []
    G_losses, D_losses, W_dist, G_score, G_max_score, G_dvalue, G_unq = [], [], [], [], [], [], []
    ...
    for mini_batch in real_dataloader:
        ...
        g_count = 0
        while 1:
            start_Ggen = timeit.default_timer()
            fake_data = sample_generator(batch_size)   # generate a mini-batch of fake data 64*10*21      
            stop_Ggen = timeit.default_timer()
            Ggen_time = stop_Ggen - start_Ggen
            # print('GgenTime: '+ str(Gstep_time) + '(s)')
            Ggen_time_each.append(Ggen_time)
            if ~torch.isnan(fake_data.sum()):
                break
            g_count = g_count + 1
            print('regenerate-d'+str(g_count))

        start_Dpred = timeit.default_timer()
        d_fake_pred = D(fake_data)    # what's the prediction you get via discriminator
        stop_Dpred = timeit.default_timer()
        Dpred_time = stop_Dpred - start_Dpred
        # print('DpredTime: '+ str(Dpred_time) + '(s)')
        Dpred_time_each.append(Dpred_time)
        d_fake_err = d_fake_pred.mean() # different here
        ...
        d_error_total = d_fake_err - d_real_err
        ...
        start_Dupdate = timeit.default_timer()
        d_error_total.backward(retain_graph=True)
        D_optimizer.step()      
        stop_Dupdate = timeit.default_timer()
        Dupdate_time = stop_Dupdate - start_Dupdate
        # print('DstepTime: '+ str(Dstep_time) + '(s)')
        Dupdate_time_each.append(Dupdate_time)     
        
        # weight clipping
        # https://github.com/minlee077/WGAN-pytorch/blob/master/notebooks/WGAN-weight_clipping.ipynb
        clip_value = 0.01
        ...
        start_CNNPred = timeit.default_timer()
        fake_scores = trainedCNN(after_pca, hla, cnn_model,fake_data)
        real_scores = trainedCNN(after_pca, hla, cnn_model,real_data)
        stop_CNNPred = timeit.default_timer()
        CNNPred_time = stop_CNNPred - start_CNNPred
        # print('CNNPredTime: '+ str(CNNPred_time) + '(s)')
        CNNPred_time_each.append(CNNPred_time)
        
        start_Spred = timeit.default_timer()
        s_fake_pred = S(fake_data)
        s_real_pred = S(real_data)
        stop_Spred = timeit.default_timer()
        Spred_time = stop_Spred - start_Spred
        # print('SPredTime: '+ str(SPred_time) + '(s)')
        Spred_time_each.append(Spred_time)
        s_error_total = Score_loss(s_fake_pred.cuda(),fake_scores.cuda()) + Score_loss(s_real_pred.cuda(),real_scores.cuda()) 
        ...
        S_losses.append(s_error_total.detach().cpu().numpy())

        # Update the parameters in the generator every d_steps(10) in mini_batch(64)
        if counter % d_steps == 0:
            start_Supdate = timeit.default_timer()
            s_error_total.backward(retain_graph=True)
            S_optimizer.step() 
            stop_Supdate = timeit.default_timer()
            Supdate_time = stop_Supdate - start_Supdate
            # print('SstepTime: '+ str(Sstep_time) + '(s)')
            Supdate_time_each.append(Supdate_time)
            
            G_optimizer.zero_grad()
            g_count = 0
            unq_count = batch_size
            while 1:
                start_Ggen = timeit.default_timer()
                ...
                if ~torch.isnan(g_fake_data.sum()):
                    generation = g_fake_data.detach().cpu().numpy()
                    # https://numpy.org/doc/stable/reference/generated/numpy.argmax.html
                    # Returns the indices of the maximum values along an axis
                    hard = np.argmax(generation, axis=2)  # [N,seq_len]
                    # https://stackoverflow.com/questions/45176383/count-occurrences-of-unique-arrays-in-array
                    # count unique generated peptides
                    unq_rows, count = np.unique(hard,axis=0, return_counts=1)
                    unq_count = len(count)
                    break
                g_count = g_count + 1
                print('regenerate-g'+str(g_count))
                
            # Create the repetition count array
            repetition_counts = count + 1
            # Convert repetition_counts to a PyTorch tensor
            counts_tensor = torch.tensor(repetition_counts, dtype=torch.float32).to(device)
                
            unq_rate = unq_count/batch_size
            ...
            # D_range = abs(torch.mean(dg_fake_pred)-torch.mean(sg_fake_pred))
            
            # score_scale = 1 + D_range
            
            # if unq_count == batch_size:
            #     gamma = 1
            # else:
            #     gamma = 0
            ...
            """
            ORGAN+MolGAN_Loss
            Divide reward with the number of repetition (repetition_counts)
            +
            Weighted sum of Critic loss and Predictor loss
            """
            g_lamda = 0
            g_error_total = -g_lamda * torch.mean(dg_fake_pred) - (1 - g_lamda)*torch.mean(sg_fake_pred/counts_tensor)
            
            start_Gupdate = timeit.default_timer()
            g_error_total.backward()
            G_optimizer.step()
            stop_Gupdate = timeit.default_timer()
            Gupdate_time = stop_Gupdate - start_Gupdate
            # print('GstepTime: '+ str(Gstep_time) + '(s)')
            Gupdate_time_each.append(Gupdate_time)
            
            g_err = g_error_total            
            G_losses.append(g_err.detach().cpu().numpy())
            G_score.append(torch.mean(g_fake_scores).detach().cpu().numpy())
            G_max_score.append(torch.max(g_fake_scores).detach().cpu().numpy())
            G_dvalue.append(torch.mean(dg_fake_pred).detach().cpu().numpy())
            G_unq.append(unq_rate)
            S.train(True)
        else:
            start_Supdate = timeit.default_timer()
    ...
    summary_string = 'Epoch{0}/{1}: d_real_loss-{2:.2f},d_fake_loss-{3:.2f},d_total_loss-{4:.2f},G_score-{5:.2f},G_total_loss-{6:.2f},W_dist-{7:.2f},S_losses-{8:.2f},G_max_score-{9:.2f},unq_rate-{10:.2f}' \
        .format(epoch + 1, num_epochs, np.mean(d_real_losses), np.mean(d_fake_losses), np.mean(D_losses),
                np.mean(G_score), np.mean(G_losses), np.mean(W_dist), np.mean(S_losses), np.mean(G_max_score), np.mean(G_unq))
    ...
    array9.append(np.mean(G_max_score))
    array10.append(np.mean(G_unq))
    array11.append(np.sum(Dupdate_time_each))
    array12.append(np.sum(Gupdate_time_each))
    array13.append(np.sum(Ggen_time_each))
    array14.append(np.sum(Dpred_time_each))
    array15.append(np.sum(CNNPred_time_each))
    array16.append(np.sum(Supdate_time_each))
    array17.append(np.sum(Spred_time_each))

    if epoch % 50 == 49 or epoch == (selected_save_epoch-1):
        total = []
        for i in range(160):
    ...
    print('Time: '+ str(stop_epoch - start_epoch) + '(s)'
          + ', DupdateTime: '+ str(np.sum(Dupdate_time_each)) + '(s)' 
          + ', GupdateTime: '+ str(np.sum(Gupdate_time_each)) + '(s)' 
          + ', GgenTime: '+ str(np.sum(Ggen_time_each)) + '(s)' 
          + ', DpredTime: '+ str(np.sum(Dpred_time_each)) + '(s)' 
          + ', CNNPredTime: '+ str(np.sum(CNNPred_time_each)) + '(s)'
          + ', SupdateTime: '+ str(np.sum(Supdate_time_each)) + '(s)' 
          + ', SpredTime: '+ str(np.sum(Spred_time_each)) + '(s)' )
    array8.append(stop_epoch - start_epoch)

stop_train = timeit.default_timer()
print('Training Time: '+ str(stop_train - start_train) + '(s)'
      + ', Total DupdateTime: '+ str(np.sum(array11)) + '(s)' 
      + ', Total GupdateTime: '+ str(np.sum(array12)) + '(s)' 
      + ', Total GgenTime: '+ str(np.sum(array13)) + '(s)' 
      + ', Total DpredTime: '+ str(np.sum(array14)) + '(s)' 
      + ', Total CNNPredTime: '+ str(np.sum(array15)) + '(s)'
      + ', Total SupdateTime: '+ str(np.sum(array16)) + '(s)' 
      + ', Total SpredTime: '+ str(np.sum(array17)) + '(s)'
      + ', Total OtherTime: '+ str(stop_train - start_train - np.sum(array11) - np.sum(array12) - np.sum(array13) - np.sum(array14) - np.sum(array15) - np.sum(array16) - np.sum(array17)) + '(s)')
# save the model
...
np.save(output_dir+'/G_max_score.npy', array9)
np.save(output_dir+'/G_unq.npy', array10)
np.save(output_dir+'/Dupdate_time_sum.npy', array11)
np.save(output_dir+'/Gupdate_time_sum.npy', array12)
np.save(output_dir+'/Ggen_time_sum.npy', array13)
np.save(output_dir+'/Dpred_time_sum.npy', array14)
np.save(output_dir+'/CNNPred_time_sum.npy', array15)
np.save(output_dir+'/Supdate_time_sum.npy', array16)
np.save(output_dir+'/Sgen_time_sum.npy', array17)
