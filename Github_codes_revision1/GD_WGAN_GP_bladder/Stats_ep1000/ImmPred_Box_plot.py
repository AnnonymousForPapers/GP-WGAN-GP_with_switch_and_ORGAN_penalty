...
output_file = "ImmPredresults.txt"

with open(output_file, "w") as f:
    for method, method_name in zip(method_array, name_array):
        if "dataset" in method_name:
            data_path = data_file + 'Results/' + method + '/deepimmuno-GANRL-bladder_cancer-scored_epoch' + str(num_epochs) + '-batch' + str(batch_size) + '.txt'
        elif "_best" in method:
            if method == "GD_WGAN_wclip_MolGAN_DScalar_Loss_MyPredictor_best":
                select_epoch = 816
            elif method == "GD_WGAN_wclip_MolGAN_Gamma0_DScalar_Loss_MyPredictor_best":
                select_epoch = 6
            elif method == "GD_WGAN_wclip_OR_MolGAN_Gamma0_DScalar_Loss_MyPredictor_best":
                select_epoch = 6
            else:
                raise
            data_path = data_file + 'Results/' + method.replace("_best","") + '/' + ep + '/best_deepimmuno-GANRL-bladder_cancer-scored_epoch' + str(select_epoch) + '-batch' + str(batch_size) + '.txt'
        else:    
            data_path = data_file + 'Results/' + method + '/' + ep + '/deepimmuno-GANRL-bladder_cancer-scored_epoch' + str(num_epochs) + '-batch' + str(batch_size) + '.txt'
        ...
        df_labled = pd.DataFrame({'Immunogenicity score':imm})
        df_labled['Method'] = method_name
        df.append(df_labled)
        # Prepare the report text
        report = (
            f"\nMethod: {method}\n"
            f"# of sequences: {len(imm)}\n"
            f"Average score: {float(sum(imm)/len(imm))}\n"
            f"Std score: {float(np.std(imm))}\n"
            f"Median score: {float(np.median(imm))}\n"
            f"Min. score: {float(min(imm))}\n"
            f"Max. score: {float(max(imm))}\n"
        )
...
sns.boxplot(data=df_concate, x="Method", y="Immunogenicity score",saturation=0.5,width=0.5,palette=color_array, ax=ax3)
sns.stripplot(data=df_concate, x="Method", y="Immunogenicity score",palette=color_array, size=1, alpha=0.5, ax=ax3)
...
for c in ax3.yaxis.get_major_ticks():
    c.label.set_fontsize(15)
plt.title('DeepImmuno',fontsize=25, weight='bold')
...
plt.savefig('ImmPred_box.png', dpi=300)
plt.show()
