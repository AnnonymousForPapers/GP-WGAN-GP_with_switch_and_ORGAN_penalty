...
with open(output_file, "w") as f:  # open file once before loop
    for method, method_name in zip(method_array, name_array):
        if "dataset" in method_name:
            data_path = data_file + 'Results/' + method + '/NetMHCpan41_prediction_epoch1000.csv'
        elif "_best" in method:
            data_path = data_file + 'Results/' + method.replace("_best","") + '/' + ep + '/NetMHCpan41_prediction_epoch1000.csv'
        else:    
            data_path = data_file + 'Results/' + method + '/' + ep + '/NetMHCpan41_prediction_epoch1000.csv'
...
for c in ax3.yaxis.get_major_ticks():
    c.label.set_fontsize(15)
plt.title('NetMHCpan4.1',fontsize=25, weight='bold')
...
plt.savefig('Net41_box.png', dpi=300)
plt.show()
