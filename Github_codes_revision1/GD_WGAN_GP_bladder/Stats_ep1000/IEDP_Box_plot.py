...
output_file = "IEDBresults.txt"

with open(output_file, "w") as f:
    for method, method_name in zip(method_array, name_array):
        if "dataset" in method_name:
            data_path = data_file + 'Results/' + method + '/result.csv'
        elif "_best" in method:
            
            data_path = data_file + 'Results/' + method.replace("_best","") + '/' + ep + '/best_result.csv'
        else:    
            data_path = data_file + 'Results/' + method + '/' + ep + '/result.csv'
...
for c in ax3.yaxis.get_major_ticks():
    c.label.set_fontsize(15)
plt.title('IEDB',fontsize=25, weight='bold')
...
plt.savefig('IEDB_box.png', dpi=300)
plt.show()
