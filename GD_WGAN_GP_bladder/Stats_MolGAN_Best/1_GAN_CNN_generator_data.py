...
for method, num_epochs  in zip(method_array, num_epochs_array):
    ...
    outname = 'best_deepimmuno-GANRL-bladder-epoch' + str(num_epochs) + '-batch' + str(batch_size) + '.txt'
    ...
    G.load_state_dict(torch.load(data_file + 'result/' + method + '/epoch' + str(epoch_file) + '/model_epoch_' + str(num_epochs - 1) + '.pth'))
