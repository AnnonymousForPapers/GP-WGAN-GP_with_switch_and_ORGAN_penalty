...
for method, num_epochs  in zip(method_array, num_epochs_array):
    ...
    #%% Set file directory
    data_file = '../../'
    outdir = data_file + 'result_brain/' + method + '/epoch' + str(epoch_file)
    print("outdir is {}".format(outdir))
    #outname = 'deepimmuno-GANRL-brain-epoch' + str(num_epochs) + '.txt'
    outname = 'best_deepimmuno-GANRL-brain-epoch' + str(num_epochs) + '-batch' + str(batch_size) + '.txt'
    ...
    G.load_state_dict(torch.load(data_file + 'result_brain/' + method + '/epoch' + str(epoch_file) + '/model_epoch_' + str(num_epochs - 1) + '.pth'))
