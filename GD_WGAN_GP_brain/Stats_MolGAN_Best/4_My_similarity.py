...
# epoch = 1000
num_epochs_array = [220,15,15] # generator trained epoch
batch_size = 10000 # 6232 peptides = len(raw_Bladder)
# ep = 'epoch' + str(epoch)
ep = 'epoch1000'
# data_path = 'D:/PhD thesis/GCN/My Code/' + method + '/' + ep + '/deepimmuno-GANRL-brain-' + ep + '_rmv.txt'
for method, num_epochs in zip(method_array, num_epochs_array):
    data_path = data_file + 'result_brain/' + method + '/' + ep + '/best_deepimmuno-GANRL-brain-epoch' + str(num_epochs) + '-batch' + str(batch_size) + '_rmv.txt'
    generate = pd.read_csv(data_path,sep='\t')
    
    #generate = pd.read_csv('D:/PhD thesis/GCN/My Code/data/df/df_all_epoch100.csv')
    #generate = pd.read_csv('D:/PhD thesis/GCN/My Code/Test/GAN_RL_no_label_result_brain/epoch0/test_generator_0.csv')
    ...
    np.save(outdir + '/best_MaxSimilarity_epoch' + str(num_epochs) + '-batch' + str(batch_size), whole2)
    
    count = 0
    #with open('D:/PhD thesis/GCN/My Code/' + method + '/' + ep + '/log_rmv_0.txt','w') as f:
    with open(data_file + 'result_brain/' + method + '/' + ep + '/best_epoch' + str(num_epochs) + '-batch' + str(batch_size) +'log_rmv_0.txt','w') as f:
        for item1 in seq:
            total = []
            for item2 in raw:
                total.append((item2,similar(item1,item2)))
