...
for method, num_epochs  in zip(method_array, num_epochs_array):
    ...
    #Bladder = pd.read_csv('D:/PhD thesis/GCN/My Code/' + method + '/' + ep + '/deepimmuno-GANRL-brain-' + ep + '.txt',sep='\t')
    Bladder = pd.read_csv(data_file + 'result_brain/' + method + '/' + ep + '/best_deepimmuno-GANRL-brain-epoch' + str(num_epochs) + '-batch' + str(batch_size) + '.txt',sep='\t')
    ...
    outname = 'best_deepimmuno-GANRL-brain-epoch' + str(num_epochs) + '-batch' + str(batch_size) + '_rmv.txt'
